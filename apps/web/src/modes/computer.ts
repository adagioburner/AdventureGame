...
/** How long the computer thinks before handing the page back to draw a frame. */
 ...
 *
 * It thinks on the page's own thread, a slice of about `SLICE_MS` at a time
 * with the thread handed back in between, so the map still pans and zooms and
 * the thinking bar still fills. A Web Worker would keep the thread entirely
 * free, but the published game page cannot count on being allowed to start
 * one; slices work wherever the page does. The thinking time is wall-clock
 * time from the moment it starts, the page's own drawing included.
 *
...
/**
 * Run `next` as a new task, after whatever the page has waiting: input, and a
 * frame when one is due. A message rather than `setTimeout(0)`, which browsers
 * hold back by 4 ms once it nests.
 */
